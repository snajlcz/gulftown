...
namespace
{
    ...
    void Teleport(Player * const player, const uint16 &map,
                  const float &X, const float &Y, const float &Z, const float &orient, const uint32 &phase)
    {
    ...
    void ActionTeleport(Player * const player, Creature * const creature, const uint32 &id)
    {
        ...
        Teleport(player, dest.m_map, dest.m_X, dest.m_Y, dest.m_Z, dest.m_orient, dest.m_phase);
    }
}
